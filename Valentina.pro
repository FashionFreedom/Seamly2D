...
unix {
    *g++* {
        ...
        contains(GCC_VERSION, ^7.*$) {
            message( "Valentina.pro: g++ version 7.x found" )
            CONFIG += g++7
        } else {
            contains(GCC_VERSION, ^6.*$) {
                message( "Valentina.pro: g++ version 6.x found" )
                CONFIG += g++6
            } else {
                contains(GCC_VERSION, ^5.*$) {
                    message( "Valentina.pro: g++ version 5.x found" )
                    CONFIG += g++5
                } else {
                    contains(GCC_VERSION, ^4.*$) {
                        message( "Valentina.pro: g++ version 4.x found" )
                        CONFIG += g++4
                    } else {
                        message( "Valentina.pro: Unknown GCC configuration" )
                    }
                }
            }
        }
