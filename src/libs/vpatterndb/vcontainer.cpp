...
quint32 VContainer::pieceIdOfPath(quint32 id) const
{
 ...
 * variable's name win over DataGObjects()'s for the same id-token (see TestIdTokenCollisionOverwrites-
 * AndLogsWarning), so if such an object lost the race for the plain name to a cutSpline/cutArc/
 * cutSplinePath segment - which has no independent identity of its own, existing only as this
 * composite variable - merely because the segment happened to parse first, its own length/angle
 * variables would display under a name DataGObjects() has never heard of instead of its real one (see
 * issue #1678). When that exact situation is detected, the segment's entry is evicted to a
 * disambiguated key instead, and the canonical object keeps the plain name it is entitled to. Two
 * arcs/curves that are BOTH independently registered and happen to generate the identical name (e.g.
 * two literal splines connecting the same two points) still disambiguate the usual way - there is no
 * way to give both the plain name, and CompositeVariableTokens's own tests rely on that.
 */
 ...
 * @param line_id the line's own persisted id (a file-stored XML attribute on the generating tool, or
