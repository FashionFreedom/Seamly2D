...
VToolCutSplinePath::VToolCutSplinePath(VAbstractPattern *doc, VContainer *data, const quint32 &id,
                                       ...
                                       const quint32 &splinePathId, const quint32 &segment1_id,
                                       const quint32 &segment2_id, const Source &typeCreation,
                                       QGraphicsItem *parent)
    : VToolCut(doc, data, id, direction, formula, lineColor, splinePathId, segment1_id, segment2_id, parent)
{
                                               ...
                                               const Source &typeCreation)
{
    ...
    if (typeCreation == Source::FromGui)
    {
        ...
        data->AddSpline(QSharedPointer<VAbstractBezier>(splPath1), segment1_id, id);
        data->AddSpline(QSharedPointer<VAbstractBezier>(splPath2), segment2_id, id);
    }
    else
    {
    ...
    if (parse == Document::FullParse)
    {
        ...
        VToolCutSplinePath *point = new VToolCutSplinePath(doc, data, id, direction, formula, lineColor,
                                                            splinePathId, segment1_id, segment2_id, typeCreation);
        scene->addItem(point);
