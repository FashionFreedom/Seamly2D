                                               ...
                                               qreal angle1Deg, qreal angle2Deg)
{
    ...
    // Standard Hobby/MetaPost velocity function. The control-point distance from
    // a knot is velocity(theta, phi) * d (the 1/3 normalisation is folded into the
    // denominator). The earlier code dropped the (1 + 0.618*cos + 0.382*cos) factor
    // and inverted the velocity, producing handles ~3x too long (overshoot).
    const qreal sq2   = qSqrt(2.0);
    const qreal sqrt5 = qSqrt(5.0);
    const qreal cA    = 0.5 * (sqrt5 - 1.0);   // ~0.618  = 1/2 (sqrt5 - 1)
    const qreal cB    = 0.5 * (3.0 - sqrt5);   // ~0.382  = 1/2 (3 - sqrt5)
    ...
    const qreal rho   = velocity(theta, phi);   // velocity at P1
    const qreal sigma = velocity(phi, theta);   // velocity at P4
    const qreal c1 = qBound(1e-4, rho   * d, d * 8.0);
    const qreal c2 = qBound(1e-4, sigma * d, d * 8.0);
